...
try:
    ...
    # Process Image
    datum = op.Datum()
    # cap = cv2.VideoCapture("E:\Desktop\openpose-1.6.0\openpose-1.6.0\examples\media\ytxs1.mp4")
    cap = cv2.VideoCapture(videoPath)
    fps = cap.get(cv2.CAP_PROP_FPS)
    size = (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    framecount = cap.get(cv2.CAP_PROP_FRAME_COUNT)
    print('Total frames in this video: ' + str(framecount))
    # videoWriter = cv2.VideoWriter("E:\Desktop\openpose-1.6.0\openpose-1.6.0\examples\output/ytxs1.avi", cv2.VideoWriter_fourcc('D', 'I', 'V', 'X'), fps, size)
    ...
    while cap.isOpened():
        ...
        if hasFrame:
            ...
            percent = lh / ls
            str2 = ""
            if (percent > 1.6 and percent < 2.2):
                print("握距适中")
                str2 = "握距适中"
            elif (percent < 1.6):
                print("握距偏窄，请加宽握距")
            ...
            str3 = ""
            if len(totalAngle) > 1:
                if ((count[-1] == 0 or len(count) == 0) and angle > 70 and totalAngle[-1] < totalAngle[-2]):
                    print("向上高度未达到，请继续向上")
                    str3 = "向上高度未达到，请继续向上"
                elif (count[-1] == 1 and angle < 70):
                    print("向上高度已达到")
                    str3 = "向上高度已达到"
                elif (count[-1] == 1 and angle < 130):
                    print("向下高度未达到，请继续向下")
                    str3 = "向下高度未达到，请继续向下"
                elif (count[-1] == 0 and angle > 130 and totalAngle[-1] > totalAngle[-2]):
                    print("向下高度已达到")
                    str3 = "向下高度已达到"
            print(count)
            find01(count)
            ...
            font1 = ImageFont.truetype('msyhbd.ttc', 36, encoding="gb2312")  # 40为字体大小，根据需要调整
            fillColor = (255, 0, 0,)
            # position = (100, 120) #第一个数值是距左，第二个数值是距
            str1 = "num: " + str(num)

            draw = ImageDraw.Draw(img_PIL)
            draw.text((100, 120), str1, font=font1, fill=fillColor)
            draw.text((100, 200), str2, font=font1, fill=fillColor)
            draw.text((100, 280), "手肘角度：" + str(angle), font=font1, fill=fillColor)
            draw.text((100, 360), str3, font=font1, fill=fillColor)
