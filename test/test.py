...
try:
    ...
    parser.add_argument("--video_path", default="", help="Process video. Read all standard formats (mp4).")
    parser.add_argument("--output_video_path", default="", help="Output video path.")
    ...
    # Process Image
    datum = op.Datum()
    #cap = cv2.VideoCapture("E:\Desktop\openpose-1.6.0\openpose-1.6.0/build/bin\media/fuwocheng.mp4")
    ...
    videoWriter = cv2.VideoWriter(output_video_path, cv2.VideoWriter_fourcc('D', 'I', 'V', 'X'), fps, size)
    ...
    while cap.isOpened():
        ...
        if hasFrame:
            ...
            angle_left_elbow(datum.poseKeypoints)
            print(count)
            find010(count)
            print("num: "+str(num))
            ...
            draw.text(position, str1, font=font1, fill=fillColor)
